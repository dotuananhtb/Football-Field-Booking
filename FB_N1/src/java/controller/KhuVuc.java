 ...
 * @author Admin
 */
@WebServlet(name = "KhuVuc", urlPatterns = {"/KhuVuc"})
public class KhuVuc extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (zoneID == null || zoneID.trim().isEmpty()) {
            list = fDao.getAllFields();
        } else {
            list = fDao.getFieldsByZoneId(zoneID);
        }

        request.setAttribute("listZ", listZ);
        request.setAttribute("listF", list);
        request.setAttribute("selectedZoneId", zoneID); // có thể null hoặc chuỗi
